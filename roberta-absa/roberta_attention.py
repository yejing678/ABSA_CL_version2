...
import copy
import numpy as np
from Attention.scaled_dot import ScaledDotAttention
...
class LCF_ROBERTA_Attention(nn.Module):
    def __init__(self, bert, opt):
        super(LCF_ROBERTA_Attention, self).__init__()
        ...
        # self.global_weight = opt.alpha_g
        # self.local_weight = opt.alpha_l
        # self.wg = nn.Parameter(torch.tensor(self.global_weight), requires_grad=False)
        # self.wl = nn.Parameter(torch.tensor(self.local_weight), requires_grad=False)
        self.attention = ScaledDotAttention(opt.dropout,opt.bert_dim)
        self.LayerNorm = nn.LayerNorm(opt.bert_dim, eps=self.bert_spc.config.layer_norm_eps)
        self.fnn = PositionwiseFeedForward(opt.bert_dim, dropout=opt.dropout)
    ...
    def forward(self, inputs):
        ...
        # bert_spc_out, _ = self.bert_spc(input_ids=text_bert_indices)
        outputs1 = self.bert_spc(input_ids=text_bert_indices)
        bert_spc_out = outputs1.last_hidden_state
        bert_spc_pooler = outputs1.pooler_output
        bert_spc_out = self.dropout(bert_spc_out)

        # bert_local_out, _ = self.bert_local(text_local_indices)
        outputs2 = self.bert_local(input_ids=text_local_indices)
        bert_local_out = outputs2.last_hidden_state
        bert_local_out = self.dropout(bert_local_out)

        bert_local_out1 = torch.zeros_like(bert_local_out).to(self.opt.device)
        if self.opt.local_context_focus == 'cdm':
            ...
            bert_local_out1 = torch.mul(bert_local_out, masked_local_text_vec)
            ...
            bert_local_out1 = torch.mul(bert_local_out, weighted_text_local_features)

        attention_out = self.attention(bert_local_out1, bert_local_out, bert_local_out)  # [16, 85, 768]
        # attention_out = self.LayerNorm(attention_out)
        # out_sum = attention_out + bert_local_out
        # out_sum = self.LayerNorm(out_sum)
        # out = self.fnn(out_sum)
        # out = self.LayerNorm(out + out_sum)
        # self_attention_out = self.bert_SA(out)
        # global_output = torch.cat((bert_spc_pooler, bert_spc_out), dim=1)
        # out_sum = attention_out + bert_spc_out
        # self_attention_out = self.bert_SA( out_sum)
        # out_sum = self.LayerNorm(out_sum)

        # mean_pool = self.linear_single(out_sum)
        out_cat = torch.cat((bert_spc_out,attention_out), dim=-1)
        out_cat = self.linear_double(out_cat)
        out_cat = self.LayerNorm(out_cat)
