...
from transformers.models.roberta.modeling_roberta import RobertaPooler, RobertaSelfAttention
...
class LCF_Roberta(nn.Module):
    def __init__(self, bert, opt):
        super(LCF_Roberta, self).__init__()
        ...
        self.bert_pooler = RobertaPooler(bert.config)
    ...
    def forward(self, inputs):
        ...
        # local_output = torch.cat((bert_local_pooler, bert_local_out), dim=1)
        out_cat = torch.cat((bert_spc_out, bert_local_out), dim=-1)
        # out_cat = torch.cat((bert_local_out, bert_spc_out), dim=-1)
        concat_hidden_states_dense = self.linear_double(out_cat)
        self_attention_out = self.bert_SA(concat_hidden_states_dense)
