...
def build_embedding_matrix(word2idx, embed_dim, dat_fname):
    if os.path.exists(dat_fname):
        ...
        fname = './glove.twitter.27B/glove.twitter.27B.' + str(embed_dim) + 'd.txt' \
            if embed_dim != 300 else "/home/jye/embedding/glove.840B.300d.txt"
        print(fname)
...
class ABSADataset(Dataset):
    def __init__(self, fname, tokenizer):
        ...
        for i in tqdm(range(0, len(lines), 4)):
            ...
            # sentence_label = lines[i + 3].strip()
            implicit = lines[i + 3].strip()
            d = {
                'Positive': '1',
                'Negative': '-1',
                'Neutral': '0'
            ...
            # sentence_label = int(sentence_label) + 1
            implicit_label = int(implicit)
            ...
            # NLI_bert_indices = tokenizer.text_to_sequence(
            #     '[CLS] ' + text_left + " " + aspect + " " + text_right + ' [SEP] ' + 'what is the sentiment of ' + aspect + " ? [SEP]")
            # NLI_segment_indices = [0] * (text_len + 1) + [1] * (aspect_len + 8)
            # NLI_segment_indices = pad_and_truncate(NLI_segment_indices, tokenizer.max_seq_len)
            ...
            left_aspect_bert_indices = tokenizer.text_to_sequence('[CLS] ' + text_left + ' ' + aspect + ' ' + ' [SEP]')
            ...
            data = {
                ...
                # 'NLI_bert_indices': NLI_bert_indices,
                # 'NLI_segment_indices': NLI_segment_indices
            }
