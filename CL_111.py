...
from Loss.CL_loss import CL_loss_arrange
...
class Instructor:
    def __init__(self, opt):
        ...
        self.trainset = ABSADataset(opt.dataset_file['train_total'], tokenizer)
    ...
    def _train_cl_gradual_stage(self, cl_loss, optimizer, train_data_loader, val_data_loader):
        print('staring train...')
        ...
        for i_epoch in tqdm(range(self.opt.num_epoch)):
            ...
            for i_batch, batch in enumerate(train_data_loader):
                ...
                logic, features = self.model(inputs)
                sentiment_labels = batch['polarity'].to(self.opt.device)
                implicit_labels = batch['implicit_label'].to(self.opt.device)

                # calculate loss
                loss = cl_loss(logic, features, sentiment_labels, implicit_labels, i_epoch, self.opt.curriculum_stages)
                loss.backward()
                optimizer.step()
    ...
    def run(self):
        # Loss and Optimizer
        loss = CL_loss_arrange(opt=self.opt, margin=self.opt.margin, hardest=False, squared=False)
        ...
        self._reset_params()
        best_model_path = self._train_cl_gradual_stage(loss, optimizer, train_data_loader, val_data_loader)
...
def main():
    ...
    parser.add_argument('--train_mode', default='CE+TL', type=str)
    ...
    parser.add_argument('--seed', default=3514, type=int, help='set seed for reproducibility')
    ...
    parser.add_argument('--curriculum_mode', default='change_sample_weight', type=str)
