...
class SelfAttention(nn.Module):
    ...
    def forward(self, inputs):
        zero_tensor = torch.tensor(np.zeros((inputs.size(0), 1, 1, self.opt.max_seq_len+1),
                                            dtype=np.float32), dtype=torch.float32).to(self.opt.device)
        SA_out = self.SA(inputs, zero_tensor)
        return self.tanh(SA_out[0])


class LCF_BERT_LOCAL_CLS(nn.Module):
    def __init__(self, bert, opt):
        super(LCF_BERT_LOCAL_CLS, self).__init__()
    ...
    def forward(self, inputs):
        ...
        bert_spc_pooler = bert_spc_pooler.unsqueeze(dim=1)
        out_cat = torch.cat((bert_spc_pooler, bert_local_out), dim=1)
...
class LCF_BERT_ONLY_GLO(nn.Module):
    def __init__(self, bert, opt):
        super(LCF_BERT_ONLY_GLO, self).__init__()
    ...
    def forward(self, inputs):
        ...
        # print(bert_local_out.size())  # [16, 85, 768]
        # print(bert_spc_out.size())  # [16, 85, 768]
        # exit(0)
        bert_spc_pooler = bert_spc_pooler.unsqueeze(dim=1)
        # print(bert_spc_pooler.size())
        out_cat = torch.cat((bert_spc_pooler,bert_spc_out), dim=1)
        # print(out_cat.size())
        # exit(0)
