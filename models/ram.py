...
class RAM(nn.Module):
    ...
    def forward(self, inputs):
        ...
        memory, (_, _) = self.bi_lstm_context(memory, memory_len)  # [16, 51, 600]

        memory = self.locationed_memory(memory, memory_len, left_len, aspect_len)   # [16, 51, 601]
        ...
        for _ in range(self.opt.hops):
            x=torch.cat([memory,
                ...
                dim=-1)
            # print(memory.size())  # [32, 51, 601]
            # print(aspect.size())  # [32, 300]
            # print(torch.zeros(batch_size, seq_len, self.opt.embed_dim).to(self.opt.device).size())  # [32, 51, 300]
            # print(x.size())  # [32, 51, 1201]
            # exit(0)
            g = self.att_linear(x.float())   # [16, 51, 1]
