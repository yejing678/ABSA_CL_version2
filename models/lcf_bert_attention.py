...
class LCF_BERT_Attention(nn.Module):
    def __init__(self, bert, opt):
        super(LCF_BERT_Attention, self).__init__()
        ...
        self.attention = ScaledDotAttention(dropout=opt.dropout, dim=opt.bert_dim)
    ...
    def forward(self, inputs):
        ...
        # bert_spc_out = self.bert_SA(bert_spc_out)
        ...
        # bert_local_out1 = self.bert_SA(bert_local_out1)
        ...
        # out_cat = self.LayerNorm(attention_out)
        '''ADD & Norm'''
        ...
        # self_attention_out = self.bert_SA(attention_out)
        ...
        # attention_out = self.LayerNorm(attention_out)
        # out_cat = torch.cat((attention_out, bert_spc_out), dim=-1)
        # mean_pool = self.linear_double(out_cat)
        # mean_pool = self.LayerNorm(mean_pool)
        # self_attention_out = self.bert_SA(out)
        self_attention_out = self.bert_SA(out_sum)
        pooled_out = self.bert_pooler(self_attention_out)
        # pooled_out = pooled_out + bert_spc_pooler
