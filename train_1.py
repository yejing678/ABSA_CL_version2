...
class Instructor:
    ...
    def _train(self, CE, TL_hard, TL_hardest, SCL, optimizer, train_data_loader, val_data_loader):
        print('staring train...')
        ...
        w_tensorboard_log = SummaryWriter(
            comment='{}-{}-{}-{}'.format(self.opt.model_name, self.opt.dataset, self.opt.train_mode, self.opt.seed))
            ...
            for i_batch, batch in enumerate(train_data_loader):
                ...
                if self.opt.train_mode == 'CE':
                    ...
                    if have_different_sample(aspect_sentiments):
                        ...
                        if max_val_acc <=0.80:
                            loss2 = TL_hard(sentence_features, aspect_sentiments)
                        else:
                            loss2 = TL_hardest(sentence_features, aspect_sentiments)
                    else:
                        loss2 = 0
                ...
                if step % self.opt.log_step == 0:
                    ...
                    val_acc, val_f1 = self._val_evaluate_acc_f1(data_loader=val_data_loader)
                    logger.info(
                    ...
                    if val_acc > max_val_acc:
                        ...
                        save_path = "state_dict/{0}/{1}/{2}/{3}/SCL:{4} TL:{5}/({6},{7},{8})/{9}/" \
                            .format(self.opt.train_mode, self.opt.dataset, self.opt.model_name,
                                    self.opt.local_context_focus, self.opt.lamda, self.opt.beta,
                                    self.opt.pos_weight, self.opt.neu_weight, self.opt.neg_weight, self.opt.seed)
    ...
    def _val_evaluate_acc_f1_loss(self, CE, TL_hard, TL_hardest, SCL, data_loader):
        n_correct, n_total = 0, 0
        t_targets_all, t_outputs_all = None, None
        loss_list = []
        # switch model to evaluation mode
        ...
        with torch.no_grad():
            for i_batch, t_batch in enumerate(data_loader):
                ...
                # calculate loss
                loss = 0
                loss1 = CE(t_outputs, t_targets)
                if self.opt.train_mode == 'CE':
                    loss = loss1
                elif self.opt.train_mode == 'CE+TL':
                    if have_different_sample(t_targets):
                        sentence_features = F.normalize(t_features, p=2, dim=1)
                        loss2 = TL_hard(sentence_features, t_targets)
                    else:
                        loss2 = 0
                    loss = loss1 + self.opt.beta * loss2
                    ...
                    if not has_opposite_labels(implicit_sentiments):
                        ...
                        normed_cls_hidden = F.normalize(t_features, dim=-1)
                        loss3 = SCL(normed_cls_hidden.unsqueeze(1), labels=implicit_sentiments)
                    loss = loss1 + self.opt.lamda * loss3
    ...
    def run(self):
        ...
        TL_hard = HardTripletLoss(opt=self.opt, margin=self.opt.margin, hardest=False)
        TL_hardest = HardTripletLoss(opt=self.opt, margin=self.opt.margin, hardest=True)
        ...
        self._reset_params()
        best_model_path = self._train(CE, TL_hard, TL_hardest, SCL, optimizer, train_data_loader, val_data_loader)
...
def main():
    ...
    dataset_files = {
        ...
        'MAMS': {
            'train': "/home/jye/ABSA/datasets/MAMS/train.txt",
            'val': "/home/jye/ABSA/datasets/MAMS/val.txt",
            'test': "/home/jye/ABSA/datasets/MAMS/test.txt"
        },
    }
    input_colses = {
        'lstm': ['text_indices'],
