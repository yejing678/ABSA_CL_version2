...
from models import IAN, MemNet, RAM, TD_LSTM, TC_LSTM, Cabasc, TNet_LF, AOA, MGAN, ASGCN, LCF_BERT
...
class Instructor:
        ...
        for i_epoch in range(self.opt.num_epoch):
            ...
            for i_batch, batch in enumerate(train_data_loader):
                ...
                if step % self.opt.log_step == 0:
                    ...
                    logger.info(
                        'global_step {}  step {}    train_loss: {:.4f}, train_acc: {:.4f}, val_acc: {:.4f}, val_f1: {:.4f}'.format(
                            global_step, step, train_loss, train_acc, val_acc, val_f1))
                    if val_acc > max_val_acc:
                        max_val_acc = val_acc
                        max_val_epoch = i_epoch
                        if not os.path.exists('state_dict'):
                            os.mkdir('state_dict')
                        path = 'state_dict/{0}_{1}_val_acc_{2}'.format(self.opt.model_name, self.opt.dataset,
                                                                       round(val_acc, 4))
        ...
        with torch.no_grad():
            for i_batch, t_batch in enumerate(data_loader):
                ...
                for i in range(len(t_batch['text'])):
                    ...
                    #   logger.info('{0} \n>>>aspect:{1}\n>>>ground_truth: {2}\n>>>predict：{3}'.format(t_batch['text'][i], t_batch['aspect'][i], t_batch['polarity'][i]-1, n_predict[i]-1))
    ...
    def run(self):
        ...
        for fid in range(self.opt.cross_val_fold):
            ...
            self._reset_params()
            best_model_path,val_acc, val_f1 = self._train(criterion, optimizer, train_data_loader, val_data_loader)
        ...
        mean_val_acc, mean_val_f1 = numpy.mean(all_val_acc), numpy.mean(all_val_f1)
        logger.info('>' * 100)
        logger.info('>>> mean_val_acc: {:.4f}, mean_val_f1: {:.4f}'.format(mean_val_acc, mean_val_f1))
...
def main():
    ...
    log_file = '{}-{}-{}-{}-{}-cross_val-{}.log'.format(opt.model_name, opt.dataset, opt.lr, opt.l2reg, opt.batch_size, strftime("%y%m%d-%H%M", localtime()))
